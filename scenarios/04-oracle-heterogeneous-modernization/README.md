...
| --- | --- | --- |
| Order management | PostgreSQL Flexible Server | 812 automatic, 133 manual, 20 errors |
| Finance ledger | Azure SQL Managed Instance | 498 automatic, 76 manual, 12 errors |
...
ledger to instance-scoped SQL, Plant maintenance stays where it is. There is no estate-wide
Oracle answer, and forcing one is how the difficult schema sinks the whole programme.
...
**No assessment means no migration recommendation.** Plant maintenance is retained. Not
because retaining is good, but because recommending a destination for unread PL/SQL behind a
vendor-supported application would be a guess with a score attached.
...
| 2 | Business critical | Finance ledger |

Plant maintenance appears in `deferred_workload_ids`. It is a decision, not a gap.

## What would break this

- Recommending a target that was never assessed for that schema.
- Recommending anything for Plant maintenance.
