...
| Order management: business validation of converted behaviour | Business users exercise the agreed scenarios and confirm the outcome | Explicit confirmation from the named business validators | Yes |

### Concurrency

| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Order management: concurrency and locking behaviour are acceptable | Run the peak concurrency profile and observe blocking and deadlocks | No sustained blocking beyond the baseline profile | Yes |
...
| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Order management: row counts and checksums reconcile | Compare row counts and column checksums per table | Zero unreconciled tables | Yes |

### Data types

| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Order management: data types and precision are preserved | Compare column types, precision, scale, and collation | Zero precision or collation differences without a recorded decision | Yes |
...
| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Order management: application functional tests pass | Run the application functional suite against the target | Zero failures | Yes |
...
| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Order management: integrations with dependent systems succeed | Exercise each confirmed dependency end to end | Every confirmed dependency succeeds | Yes |
...
| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Order management: performance is within tolerance of baseline | Replay the baseline workload and compare latency and throughput | Within 10% of the recorded baseline | Yes |
...
| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Order management: converted code produces identical results | Run the agreed comparison set against source and target and diff the output. Conversion tooling statistics are not accepted as evidence here. | Identical results for every case in the comparison set | Yes |
| Order management: stored procedures and functions behave identically | Execute the agreed procedure test set and compare results with the source | Identical results for every case in the set | Yes |

### Referential integrity

| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Order management: referential integrity holds | Validate constraints and check for orphaned rows | Zero constraint violations and zero orphans | Yes |

### Regression

| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Order management: regression suite passes | Run the application regression suite against the target | Zero new failures against the source run | Yes |
...
| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Order management: schema objects reconcile | Compare object counts and definitions between source and target | Zero unexplained differences | Yes |
