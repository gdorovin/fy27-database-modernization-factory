...
| Target decisions | td-wl-order-management |
...
| --- | --- | --- | --- | --- |
| Order management | azure-database-for-postgresql-flexible-server | refactor | Yes | Yes |
...
- **Provision the non-production target environment** (`wave-1-pilot-t-provision-nonprod`, build) — owner architect
...
- **Seed the non-production target** (`wave-1-pilot-t-seed-nonprod`, migrate) — owner database-owner
  - Rollback: Stop replication and drop the non-production target databases.
- **Rehearse the cutover and measure the window** (`wave-1-pilot-t-rehearse-cutover`, migrate) — owner delivery-lead
  - Rollback: Restore the non-production target from the pre-rehearsal snapshot.
- **Rehearse the rollback** (`wave-1-pilot-t-rehearse-rollback`, migrate) — owner operations-owner
  - Rollback: This task is the rollback rehearsal; failure means the plan is not ready.
- **Provision the production target** (`wave-1-pilot-t-production-provision`, cutover) — owner architect
  - Rollback: Leave the source serving traffic and remove the unused production target. No traffic has moved at this point.
  - Dry run first: `az deployment group what-if --template-file infra/bicep/main.bicep`
- **Execute the cutover** (`wave-1-pilot-t-execute-cutover`, cutover) — owner delivery-lead
...
| Confirm application dependencies with their owners | prepare | application-owner | wave-1-pilot-t-landing-zone | No | No |
...
- Identifier: `wave-1-pilot-t-landing-zone`
- Owner: architect
...
- Identifier: `wave-1-pilot-t-baseline`
...
- Identifier: `wave-1-pilot-t-dependency-confirm`
...
| Task | Kind | Owner | Depends on | Changes environment | Approval |
| --- | --- | --- | --- | --- | --- |
| Provision the non-production target environment | build | architect | wave-1-pilot-t-landing-zone | Yes | Yes |
| Convert schema and database code | build | database-owner | wave-1-pilot-t-provision-nonprod | No | No |
| Remediate the application for the target | build | application-owner | wave-1-pilot-t-provision-nonprod | No | No |
...
- Identifier: `wave-1-pilot-t-provision-nonprod`
- Owner: architect
...
Run conversion tooling, then hand-remediate every object the tooling could not convert. A conversion percentage is tool output, not evidence of equivalent behaviour.

- Identifier: `wave-1-pilot-t-convert-schema`
- Owner: database-owner

#### Remediate the application for the target

Update connection handling, retry logic, driver versions, and any code that relied on instance-scoped behaviour that the target does not offer.

- Identifier: `wave-1-pilot-t-application-remediation`
- Owner: application-owner
...
| Task | Kind | Owner | Depends on | Changes environment | Approval |
| --- | --- | --- | --- | --- | --- |
| Seed the non-production target | migrate | database-owner | wave-1-pilot-t-provision-nonprod | Yes | Yes |
| Rehearse the cutover and measure the window | migrate | delivery-lead | wave-1-pilot-t-seed-nonprod | Yes | Yes |
| Rehearse the rollback | migrate | operations-owner | wave-1-pilot-t-rehearse-cutover | Yes | Yes |

#### Seed the non-production target

Perform an initial data load and start continuous replication.

- Identifier: `wave-1-pilot-t-seed-nonprod`
- Owner: database-owner
- Rollback: Stop replication and drop the non-production target databases.
...
- Identifier: `wave-1-pilot-t-rehearse-cutover`
- Owner: delivery-lead
- Rollback: Restore the non-production target from the pre-rehearsal snapshot.
...
- Identifier: `wave-1-pilot-t-rehearse-rollback`
...
| Task | Kind | Owner | Depends on | Changes environment | Approval |
| --- | --- | --- | --- | --- | --- |
| Execute the validation plan | validate | delivery-lead | wave-1-pilot-t-rehearse-rollback | No | No |
| Review validation evidence and decide go / no-go | validate | architect | wave-1-pilot-t-run-validation | No | No |
...
- Identifier: `wave-1-pilot-t-run-validation`
- Owner: delivery-lead
- Dry run: `dbmodernize render-report --wave wave-1-pilot`
...
- Identifier: `wave-1-pilot-t-review-validation`
- Owner: architect
...
| Task | Kind | Owner | Depends on | Changes environment | Approval |
| --- | --- | --- | --- | --- | --- |
| Provision the production target | cutover | architect | wave-1-pilot-t-review-validation | Yes | Yes |
| Execute the cutover | cutover | delivery-lead | wave-1-pilot-t-production-provision | Yes | Yes |
...
- Identifier: `wave-1-pilot-t-production-provision`
- Owner: architect
- Dry run: `az deployment group what-if --template-file infra/bicep/main.bicep`
- Rollback: Leave the source serving traffic and remove the unused production target. No traffic has moved at this point.
...
- Identifier: `wave-1-pilot-t-execute-cutover`
- Owner: delivery-lead
- Rollback: Invoke the documented rollback: restore application configuration to the source, confirm write availability, and reconcile any data written to the target during the window.
...
| Task | Kind | Owner | Depends on | Changes environment | Approval |
| --- | --- | --- | --- | --- | --- |
| Run the hypercare period | stabilize | operations-owner | wave-1-pilot-t-execute-cutover | No | No |
| Decide whether to decommission the source | decommission | business-owner | wave-1-pilot-t-hypercare | No | No |
...
- Identifier: `wave-1-pilot-t-hypercare`
- Owner: operations-owner
...
- Identifier: `wave-1-pilot-t-decommission-decision`
...
Validation for this wave is defined in `validation-plan-1.md`. A blocking check
