...
| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Partner integration: concurrency and locking behaviour are acceptable | Run the peak concurrency profile and observe blocking and deadlocks | No sustained blocking beyond the baseline profile | Yes |
...
| Partner integration: required extensions are present and at a compatible version | Verify availability and version of each extension on the target: pg_stat_statements, pgcrypto | Every required extension present at a compatible version | Yes |
...
| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Partner integration: row counts and checksums reconcile | Compare row counts and column checksums per table | Zero unreconciled tables | Yes |

### Data types

| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Partner integration: data types and precision are preserved | Compare column types, precision, scale, and collation | Zero precision or collation differences without a recorded decision | Yes |
...
| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Partner integration: application functional tests pass | Run the application functional suite against the target | Zero failures | Yes |
...
| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Partner integration: integrations with dependent systems succeed | Exercise each confirmed dependency end to end | Every confirmed dependency succeeds | Yes |
...
| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Partner integration: performance is within tolerance of baseline | Replay the baseline workload and compare latency and throughput | Within 10% of the recorded baseline | Yes |
...
| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Partner integration: stored procedures and functions behave identically | Execute the agreed procedure test set and compare results with the source | Identical results for every case in the set | Yes |

### Referential integrity

| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Partner integration: referential integrity holds | Validate constraints and check for orphaned rows | Zero constraint violations and zero orphans | Yes |

### Regression

| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Partner integration: regression suite passes | Run the application regression suite against the target | Zero new failures against the source run | Yes |
...
| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Partner integration: schema objects reconcile | Compare object counts and definitions between source and target | Zero unexplained differences | Yes |
...
| Partner integration: controls for GDPR are in place and evidenced | Collect control evidence for each scope and review with the security owner | Every required control evidenced | Yes |
