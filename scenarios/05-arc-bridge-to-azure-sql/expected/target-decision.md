...
| As at | 2026-07-06 |
| Recommendations | 4 |
| Workloads with no recommendation | 2 |
...
- **azure-sql-database** — Cannot be recommended while blocking evidence is open. Choosing a destination now would commit the engagement to a decision the evidence does not support.
  - Blockers: wl-acquired-unit-b-sales-r-dependencies-unknown
...
- Sizing figures are estimates. No capacity or cost commitment can rest on them until a performance baseline has been collected.

#### Data residency

- Data residency requirement recorded as us-central.
...
Azure-sql-managed-instance scored highest (95) for Group finance on the published comparison. Instance-scoped features in use are available at instance scope: linked-servers, sql-agent (+25); Broad engine compatibility limits application change (+12); Managed operations remove patching and backup toil (+8). This is a recommendation and requires architecture review before it becomes a decision.
...
| azure-sql-database | blocked | 0.0 | Blocked: Instance-scoped features are in use and are not available at database scope: linked-servers, sql-agent |
| azure-sql-database-hyperscale | blocked | 0.0 | Blocked: Instance-scoped features are in use and are not available at database scope: linked-servers, sql-agent |
| azure-sql-managed-instance | recommended | 95.0 | Instance-scoped features in use are available at instance scope: linked-servers, sql-agent (+25); Broad engine compatibility limits application change (+12); Managed operations remove patching and backup toil (+8) |
...
- **azure-sql-database** — Blocked: Instance-scoped features are in use and are not available at database scope: linked-servers, sql-agent.
  - Blockers: Instance-scoped features are in use and are not available at database scope: linked-servers, sql-agent
- **azure-sql-database-hyperscale** — Blocked: Instance-scoped features are in use and are not available at database scope: linked-servers, sql-agent.
  - Blockers: Instance-scoped features are in use and are not available at database scope: linked-servers, sql-agent
...
- Instance-scoped features observed in use: linked-servers, sql-agent.
...
No destination can be recommended for Legacy warehouse while blocking evidence is open, so every migration target is blocked. azure-arc-enabled-sql-server is proposed as an interim posture only: it governs the workload where it stands and does not move it. It must not be recorded as completed modernization. Evidence is incomplete, and Arc gives continuous inventory, assessment, and governance while that is fixed (+25); Governance and security posture improve without moving the workload (+5); Arc enablement is a bridge, not a completed modernization; the workload has not moved.
...
- Open questions:
  - Dependency discovery is incomplete, so the consumers of this database and the blast radius of a cutover are unknown. This must be resolved before a destination can be recommended. (owner: database-owner, blocking)
  - Evidence sources disagree about: data_size_gb. The repository will not choose between them. This must be resolved before a destination can be recommended. (owner: database-owner, blocking)
...
Azure-sql-database scored highest (75) for Shared services on the published comparison. No instance-scoped dependencies, so database-level isolation fits (+15); Managed operations remove patching and backup toil (+10). This is a recommendation and requires architecture review before it becomes a decision.
