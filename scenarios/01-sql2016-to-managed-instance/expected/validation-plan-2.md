...
| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Customer portal: concurrency and locking behaviour are acceptable | Run the peak concurrency profile and observe blocking and deadlocks | No sustained blocking beyond the baseline profile | Yes |
| Payments core: concurrency and locking behaviour are acceptable | Run the peak concurrency profile and observe blocking and deadlocks | No sustained blocking beyond the baseline profile | Yes |
...
| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Customer portal: row counts and checksums reconcile | Compare row counts and column checksums per table | Zero unreconciled tables | Yes |
| Payments core: row counts and checksums reconcile | Compare row counts and column checksums per table | Zero unreconciled tables | Yes |

### Data types

| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Customer portal: data types and precision are preserved | Compare column types, precision, scale, and collation | Zero precision or collation differences without a recorded decision | Yes |
| Payments core: data types and precision are preserved | Compare column types, precision, scale, and collation | Zero precision or collation differences without a recorded decision | Yes |
...
| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Customer portal: application functional tests pass | Run the application functional suite against the target | Zero failures | Yes |
| Payments core: application functional tests pass | Run the application functional suite against the target | Zero failures | Yes |
...
| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Customer portal: integrations with dependent systems succeed | Exercise each confirmed dependency end to end | Every confirmed dependency succeeds | Yes |
| Payments core: integrations with dependent systems succeed | Exercise each confirmed dependency end to end | Every confirmed dependency succeeds | Yes |
...
| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Customer portal: performance is within tolerance of baseline | Replay the baseline workload and compare latency and throughput | Within 10% of the recorded baseline | Yes |
| Payments core: performance is within tolerance of baseline | Replay the baseline workload and compare latency and throughput | Within 10% of the recorded baseline | Yes |
...
| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Customer portal: stored procedures and functions behave identically | Execute the agreed procedure test set and compare results with the source | Identical results for every case in the set | Yes |
| Payments core: stored procedures and functions behave identically | Execute the agreed procedure test set and compare results with the source | Identical results for every case in the set | Yes |

### Referential integrity

| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Customer portal: referential integrity holds | Validate constraints and check for orphaned rows | Zero constraint violations and zero orphans | Yes |
| Payments core: referential integrity holds | Validate constraints and check for orphaned rows | Zero constraint violations and zero orphans | Yes |

### Regression

| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Customer portal: regression suite passes | Run the application regression suite against the target | Zero new failures against the source run | Yes |
| Payments core: regression suite passes | Run the application regression suite against the target | Zero new failures against the source run | Yes |
...
| Check | Method | Tolerance | Blocking |
| --- | --- | --- | --- |
| Customer portal: schema objects reconcile | Compare object counts and definitions between source and target | Zero unexplained differences | Yes |
| Payments core: schema objects reconcile | Compare object counts and definitions between source and target | Zero unexplained differences | Yes |
...
| Payments core: controls for PCI-DSS are in place and evidenced | Collect control evidence for each scope and review with the security owner | Every required control evidenced | Yes |
